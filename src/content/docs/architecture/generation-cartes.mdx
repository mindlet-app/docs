---
title: Génération de cartes - Pipeline complet
description: Pipeline de génération de cartes de bout en bout, du frontend mobile au service IA
sidebar:
  order: 7
---
...
<Badge text="Backend" variant="note" />
<Badge text="Service IA" variant="caution" />
<Badge text="WebSocket" variant="tip" />
...
<MermaidDownload title="Pipeline de génération de cartes">
```mermaid
sequenceDiagram
    autonumber
...
<Aside type="note" title="Communication asynchrone">
  La génération de cartes est un processus **asynchrone**. L'utilisateur reçoit immédiatement un accusé de réception (HTTP 202) puis est notifié en temps réel via WebSocket à chaque étape du processus.
</Aside>

---

## Étape 1 : Frontend Mobile
...
<MermaidDownload title="Résumé du flux IA">
```mermaid
flowchart LR
...
<MermaidDownload title="Architecture complète - Génération de cartes">
```mermaid
flowchart TB
...
<Aside type="tip" title="Pourquoi autant d'événements ?">
  Ces événements granulaires permettent au frontend d'afficher une **progression détaillée** à l'utilisateur. Chaque étape peut prendre plusieurs secondes, et informer l'utilisateur réduit l'impression d'attente.
</Aside>

---

## Introduction à l'architecture agentic
