...
description: Stratégie de tests et assurance qualité
sidebar:
  order: 3
---
...
<Aside type="note">
  Suite à l'échec de notre lancement beta, nous avons complètement revu notre approche des tests. **La qualité avant la quantité de fonctionnalités**.
</Aside>

Notre stratégie de tests repose sur la pyramide des tests :

<MermaidDownload title="Pyramide des tests">
```mermaid
flowchart TB
    subgraph Pyramid[" "]
        direction TB
        ...
        Unit["⚙️ Tests unitaires - 70%\nFonctions et classes isolées"]
    end
    
    E2E --> Integration --> Unit
    
    style E2E fill:#c62828,stroke:#ef5350
    style Integration fill:#f9a825,stroke:#fbc02d
    style Unit fill:#2e7d32,stroke:#4caf50
    style Pyramid fill:transparent,stroke:transparent
```
</MermaidDownload>
...
<Tabs>
  ...
  <TabItem label="Test d'intégration">
    ```php
...
<Aside type="tip">
  **Conseil** : Écrire les tests AVANT les fonctionnalités (TDD) permet de concevoir un code plus robuste et maintenable.
</Aside>
